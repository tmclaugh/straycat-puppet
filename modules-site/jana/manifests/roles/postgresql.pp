...
class jana::roles::postgresql {
  ...
  class { '::jana::svc::postgresql': }
  class { '::jana::svc::phppgadmin':
    require => Class['::jana::svc::postgresql']
  }

  ::jana::svc::postgresql::db { $pgsql_db_name:
    db_user   => $pgsql_db_user,
    db_passwd => $pgsql_db_passwd
  }

  ::jana::svc::postgresql::db { $foreman_db_name:
