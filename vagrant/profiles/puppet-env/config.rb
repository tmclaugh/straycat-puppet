Vagrant.configure('2') do |config|
  ...
  config.vm.define "bootstrap", primary: true do |pm|
    pm.vm.hostname = 'bootstrap.jana.local'
    ...
    pm.vm.provision 'default_puppet', type: 'puppet' do |puppet|
      ...
      puppet.facter = {
        ...
        "data_center"            => "local",
        "jm_puppetmaster_server" => "localhost",
        "os"                     => "centos6",
        "provider"               => "virtualbox",
  ...
  config.vm.define "pgsql" do |pgsql|
    pgsql.vm.hostname = 'pgsql.jana.local'
    ...
    pgsql.vm.provision 'default_puppet_server', type: 'puppet_server' do |p|
      p.puppet_server = "bootstrap.jana.local"
      p.options       = "--verbose --waitforcert 1"
      p.facter        = { 'role' => 'jana::roles::postgresql' }
    end
  end

  config.vm.define "foreman" do |foreman|
    foreman.vm.hostname = 'foreman.jana.local'
    ...
    foreman.vm.provision 'default_puppet_server', type: 'puppet_server' do |p|
      p.puppet_server = "bootstrap.jana.local"
      p.options       = "--verbose --waitforcert 1"
      p.facter        = { 'role' => 'jana::roles::foreman' }
    end
  end
  ...
  config.vm.define "puppetmaster" do |puppet|
    ...
    puppet.vm.provision 'default_puppet_server', type: 'puppet_server' do |p|
      p.puppet_server = "bootstrap.jana.local"
      p.options       = "--verbose --waitforcert 1"
      p.facter        = { 'role'                => 'jana::roles::puppetmaster',
                          'jm_foreman_server'   => 'foreman.jana.local',
                          'jm_puppet_ca_server' => 'puppetca.jana.local',
                          'jm_puppet_env'       => 'qa',
                          'jm_puppet_altnames'  => "puppetmaster.jana.local,#{$inst_name}" }
    end
  end
end